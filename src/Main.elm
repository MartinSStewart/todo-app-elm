...
update msg model =
    case msg of
            ...
            ( { model
                | todos =
                    List.append model.todos
                        [ (TodoItem "" (TodoId model.lastId) False (Color 255 255 255)) ]
        ...
        ToggleTodo todoItem ->
            ( { model
                | todos = replaceById { todoItem | done = not todoItem.done } model.todos
              }
            , Cmd.none
            )
...
todoView colorPalette selected todoItem =
    ...
    in
        div
            [ style
                [ ( "backgroundColor", colorToString todoItem.color ) ]
            , onEvent "click" <| SelectTodo <| Maybe.Just todoItem.id
            ]
        <|
            [ input
                [ type_ "checkbox"
            ...
            , input
                ...
                []
            , button
                [ onEvent "click" <| RemoveTodo todoItem ]
                [ text "✕" ]
            ]
                ++ todoBody
...
view model =
    ...
    <|
        [ button
            [ onEvent "click" AddTodo ]
            [ text "+" ]
        , button
            [ onEvent "click" DoAll ]
            [ text "Finish All" ]
        , button
            [ onEvent "click" RemoveFinished, disabled (List.all (\x -> not x.done) model.todos) ]
        ...
        , button
            ...
            ++ (List.map
                    (\x -> todoView model.colorPalette ((Maybe.Just x.id) == model.selectedTodo) x)
                    model.todos
               )
